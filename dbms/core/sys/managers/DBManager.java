...
public class DBManager {
    ...
    public class SQL {
        protected void analyzeStmt(TCustomSqlStatement stmt) throws SQLError, IOException, RecordStatus {
        ...
        protected void printConstraint(TConstraint constraint, Boolean outline) {
            ...
            switch (constraint.getConstraint_type()) {
                ...
                case reference:
                    ...
                    if (constraint.getReferencedColumnList() != null) {
                        ...
                        String fk = (constraint.getReferencedObject().toString() + "." + lcstr).replace("dmd_db.", "");
                        System.out.println("\t\treferenced columns:" + fk);
                    }
                    break;
                default:
                    break;
        ...
        protected void printObjectNameList(TObjectNameList objList) {
        ...
        protected void printColumnDefinitionList(TColumnDefinitionList cdl) {
            for (int i = 0; i < cdl.size(); i++) {
                System.out.println(cdl.getColumn(i).getColumnName());
            }
        }

        protected void printConstraintList(TConstraintList cnl) {
            for (int i = 0; i < cnl.size(); i++) {
                printConstraint(cnl.getConstraint(i), true);
            }
        }

        protected void printAlterTableOption(TAlterTableOption ato) {
        ...
        protected void analyzeCreateViewStmt(TCreateViewSqlStatement pStmt) {
        ...
        protected void analyzeInsertStmt(TInsertSqlStatement pStmt) {
            if (pStmt.getTargetTable() != null) {
                System.out.println("TableSchema name:" + pStmt.getTargetTable().toString());
            }
        ...
        protected void analyzeUpdateStmt(TUpdateSqlStatement pStmt) {
            System.out.println("TableSchema Name:" + pStmt.getTargetTable().toString());
        ...
        protected void analyzeAlterTableStmt(TAlterTableStatement pStmt) {
            System.out.println("TableSchema Name:" + pStmt.getTableName().toString());
        ...
        protected void analyzeCreateTableStmt(TCreateTableSqlStatement pStmt) throws SQLError, IOException, RecordStatus {
            String tbl_name = pStmt.getTargetTable().toString().replace("dmd_db.", "");

            TableSchema table = new TableSchema(tbl_name, pStmt.getColumnList().size());

            System.out.println("TableSchema Name:" + tbl_name);
            System.out.println("Columns:");
            ...
            for (int i = 0; i < pStmt.getColumnList().size(); i++) {
                ...
                // CREATE ATTRIBUTES
                table.attributes[i] = new Attribute(column.getColumnName().toString());
                table.attributes[i].setType((byte) Attribute.getType(column.getDatatype().toString()));

                if (column.getDefaultExpression() != null) {
                    System.out.println("\tdefault:" + column.getDefaultExpression().toString());

                    // SET DEFAULT VALUE
                    table.attributes[i].setFlag(Attribute.F_DV);
                    table.attributes[i].setDefaultValue((byte[]) column.getDefaultExpression().getVal());
                }
                if (column.isNull()) {
         ...
         * @param query
         */
        public void execute(String query) throws SQLError, IOException, RecordStatus {
